/* ==========================================================================
   Cozy Jeff Bernat Music Lounge - Core Application Engine (Hybrid Edition)
   ========================================================================== */
...
let favorites = JSON.parse(localStorage.getItem('jeff_bernat_favorites')) || [];
...
function onPlayerReady(event) {
  isYtReady = true;
  useFallbackAudio = false; // YouTube works, disable fallback!
...
function loadTrack(index) {
  ...
  if (elVinylLabel) elVinylLabel.style.backgroundImage = `url('${track.artworkUrl100}')`;
  
  if (elCurrentTimeDisplay) elCurrentTimeDisplay.textContent = "0:00";
  
  // Set duration text based on play engine
...
function playAudio() {
  ...
  // YouTube player path (If loaded and adblock isn't forcing audio fallback)
  if (isYtReady && ytPlayer && !useFallbackAudio) {
    isPlaying = true;
    ...
    if (elPlayBtn) elPlayBtn.innerHTML = '<i class="fa-solid fa-pause"></i>';
    if (elVinylRecord) elVinylRecord.classList.add('playing');
    if (elTonearm) elTonearm.classList.add('active');
  } else {
    // 100% Robust Fallback Path (Plays iTunes High-quality Previews)
    console.log("Playing via HTML5 Audio Backup Engine.");
    useFallbackAudio = true; // Lockdown fallback
...
function pauseAudio() {
  isPlaying = false;
  if (elPlayBtn) elPlayBtn.innerHTML = '<i class="fa-solid fa-play"></i>';
  if (elVinylRecord) elVinylRecord.classList.remove('playing');
  if (elTonearm) elTonearm.classList.remove('active');
  stopProgressPolling();
...
function renderLibrary() {
  ...
  filteredTracks.forEach(track => {
    ...
    item.innerHTML = `
      <div class="song-item-cover" style="background-image: url('${track.artworkUrl100}')">
...
function renderAlbumShelf() {
  ...
  uniqueAlbums.slice(0, 10).forEach(album => {
    ...
    card.innerHTML = `
      <div class="album-card-art" style="background-image: url('${album.artworkUrl100}')">
...
async function fetchSongsFromiTunes() {
  try {
    const response = await fetch('https://itunes.apple.com/search?term=jeff+bernat&limit=35&entity=song');
    ...
    if (data.results && data.results.length > 0) {
      ...
      const cleanTracks = apiTracks.filter(t => {
